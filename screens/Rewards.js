...
export default function Rewards({ navigation }) {
  ...
  const [userPoints, setUserPoints] = useState(mockUserEcoPoints);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f8fffe' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 44, paddingBottom: 16, backgroundColor: '#00C896', paddingHorizontal: 10, marginBottom: 10 }}>
        <TouchableOpacity onPress={() => navigation && navigation.goBack && navigation.goBack()} style={{ padding: 6, marginRight: 10 }}>
          <Ionicons name="arrow-back" size={28} color="#fff" />
        </TouchableOpacity>
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={{ color: '#fff', fontSize: 24, fontWeight: 'bold', letterSpacing: 1 }}>EcoPoints Rewards</Text>
        </View>
      </View>
      {/* User Points */}
      <View style={{ alignItems: 'center', marginBottom: 18 }}>
        <LinearGradient colors={["#e0f7fa", "#fff"]} style={styles.pointsCard}>
          ...
          <Text style={{ color: '#1B5E20', fontWeight: 'bold', fontSize: 28 }}>{userPoints}</Text>
        </LinearGradient>
      </View>
      {/* Rewards List */}
      <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 40 }}>
        {rewards.map((reward) => (
          <LinearGradient key={reward.id} colors={["#fff", "#e0f7fa"]} style={styles.rewardCard}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <View style={{ flex: 1, marginLeft: 16 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6 }}>
                  ...
                  <Text style={styles.rewardCost}>{reward.cost} Eco Points</Text>
                </View>
              </View>
              <TouchableOpacity
              ...
              >
                <Text style={[styles.redeemButtonText, { opacity: userPoints < reward.cost ? 0.5 : 1 }]}>Redeem</Text>
              </TouchableOpacity>
            </View>
          </LinearGradient>
        ))}
      ...
      <Modal visible={showModal} transparent animationType="fade" onRequestClose={() => setShowModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Ionicons name="gift-outline" size={44} color="#00C896" style={{ marginBottom: 10 }} />
            <Text style={{ fontSize: 20, fontWeight: 'bold', color: '#1B5E20', marginBottom: 8 }}>Confirm Redemption</Text>
            <Text style={{ fontSize: 16, color: '#333', marginBottom: 16 }}>
              Redeem <Text style={{ color: '#00C896', fontWeight: 'bold' }}>{selectedReward?.name}</Text> for <Text style={{ color: '#00C896', fontWeight: 'bold' }}>{selectedReward?.cost}</Text> Eco Points?
            </Text>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%' }}>
              <TouchableOpacity style={[styles.modalButton, { backgroundColor: '#e0e0e0' }]} onPress={() => setShowModal(false)}>
                <Text style={{ color: '#333', fontWeight: 'bold' }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modalButton, { backgroundColor: '#00C896' }]} onPress={confirmRedeem}>
