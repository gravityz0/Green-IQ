  ...
  const handleScanProduct = async (barcode) => {
    try {
      const res = await retryFn(
        () => axios.post("https://trash2treasure-backend.onrender.com/scanProduct", { barcode }),
  ...
  const handleBarcodeScanned = async ({ type, data }) => {
    try {
      ...
      if (deepSeekCache.current.has(barcodeKey)) {
        ...
        try {
          const deepSeekAnalysis = await retryFn(() => deepSeekRecommendation(json.product), { retries: 2, baseDelay: 800 });
          let youtubeId = "";
          try {
          ...
          if (isMounted.current) {
            setAnalysis("Detailed analysis is temporarily unavailable. Please try again later.");
            setVideoId("");
          }
        }
      }
      ...
      const msg = error?.response?.data?.message || error?.message || "An error occurred while scanning";
